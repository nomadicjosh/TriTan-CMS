...
class Object_Cache
{
    ...
    public function __construct($driver, \Liten\Liten $liten = null)
    {
        ...
        if ($driver == 'file') {
            $this->_cache = new \TriTan\Cache\Cache_Filesystem();
        } elseif ($driver == 'apc') {
            $this->_cache = new \TriTan\Cache\Cache_APC();
            ...
            $this->_cache = new \TriTan\Cache\Cache_Cookie();
        } elseif ($driver == 'json') {
            $this->_cache = new \TriTan\Cache\Cache_JSON();
        } elseif ($driver == 'memory') {
            $this->_cache = new \TriTan\Cache\Cache_Memory();
        }
