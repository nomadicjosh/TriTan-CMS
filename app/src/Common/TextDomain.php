...
class TextDomain
{
     ...
     */
    public function loadCoreLocale()
    {
        if (is_readable(BASE_PATH . 'config.php')) {
            $locale = $this->option->{'read'}('ttcms_core_locale');
        } else {
            $locale = 'en';
        }
        return $this->hook->{'applyFilter'}('core_locale', $locale);
    }
     ...
     */
    public function loadTextDomain($domain, $path)
    {
        /**
         ...
         * @param string $path   Path to the .mo file.
         */
        $plugin_override = $this->hook->{'applyFilter'}('override_load_textdomain', false, $domain, $path);
         ...
         * @param string $path Path to the .mo file.
         */
        $this->hook->{'doAction'}('load_textdomain', $domain, $path);
         ...
         * @param string $domain Text domain. Unique identifier for retrieving translated strings.
         */
        $mofile = $this->hook->{'applyFilter'}('load_textdomain_mofile', $path, $domain);
     ...
     * @return bool True on success, false on failure.
     */
    public function loadDefaultTextDomain($domain, $path)
    {
        $locale = $this->loadCoreLocale();

        $mopath = $path . $domain . '-' . $locale . '.mo';

        $return = $this->loadTextDomain($domain, $mopath);
    ...
    public function loadPluginTextDomain($domain, $plugin_rel_path = false)
    {
         ...
         * @param string $domain Text domain. Unique identifier for retrieving translated strings.
         */
        $plugin_locale = $this->hook->{'applyFilter'}('plugin_locale', $locale, $domain);

        if ($plugin_rel_path !== false) {
            $path = TTCMS_PLUGIN_DIR . $plugin_rel_path . DS;
        } else {
            $path = TTCMS_PLUGIN_DIR;
        }

        $mofile = $path . $domain . '-' . $plugin_locale . '.mo';
        if ($loaded = $this->loadTextDomain($domain, $mofile)) {
            return $loaded;
        }
