...
$app->group('/admin', function () use ($app, $current_user) {
    ...
    $app->match('GET|POST', '/site/', function () use ($app, $current_user) {
        if ($app->req->isPost()) {
            ...
            if (Core\is_ttcms_error($site)) {
                Cascade::getLogger('error')->{'error'}(sprintf('ERROR[%s]: %s', $site->getErrorCode(), $site->getErrorMessage()));
                Dependency\_ttcms_flash()->{'error'}(sprintf('ERROR[%s]: %s', $site->getErrorCode(), $site->getErrorMessage()));
            } else {
    ...
    $app->match('GET|POST', '/site/(\d+)/', function ($id) use ($app, $current_user) {
        if ($app->req->isPost()) {
            ...
            if (Core\is_ttcms_error($site_id)) {
                Cascade::getLogger('error')->{'error'}(sprintf('ERROR[%s]: %s', $site_id->getErrorCode(), $site_id->getErrorMessage()));
                Dependency\_ttcms_flash()->{'error'}(sprintf('ERROR[%s]: %s', $site_id->getErrorCode(), $site_id->getErrorMessage()));
            } else {
    ...
    $app->get('/site/(\d+)/d/', function ($id) use ($current_user) {
        ...
        if (Core\is_ttcms_error($site)) {
            Dependency\_ttcms_flash()->{'error'}(sprintf('ERROR[%s]: %s', $site->getErrorCode(), $site->getErrorMessage()), Core\get_base_url() . 'admin/site/');
        } else {
    ...
    $app->post('/site/users/(\d+)/d/', function ($id) use ($app, $current_user) {
        ...
        if (Core\is_ttcms_error($site_user)) {
            Dependency\_ttcms_flash()->{'error'}(sprintf('ERROR[%s]: %s', $site_user->getErrorCode(), $site_user->getErrorMessage()), Core\get_base_url() . 'admin/site/users/');
        } else {
