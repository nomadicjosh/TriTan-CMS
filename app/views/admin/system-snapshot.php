    ...
    <section class="content">
        ...
        <div class="box box-default">
            <pre>
                ...
                $report .= 'API Key:' . "\t\t\t\t\t\t" . (preg_match('/\s/', $this->app->hook->{'get_option'}('api_key')) ? '<font color="red">' . _t('No') . '</font>' : '<font color="green">' . _t('Yes') . '</font>') . PHP_EOL;
                ...
                $report .= 'Cookie Path:' . "\t\t\t\t\t\t" . (ttcms_is_writable($this->app->config('cookies.savepath')) ? '<font color="green">' . $this->app->config('cookies.savepath') . '</font>' : '<font color="red">' . $this->app->config('cookies.savepath') . '</font>') . PHP_EOL;
                $report .= 'Regular Cookie TTL:' . "\t\t\t\t\t" . ttcms_seconds_to_time($this->app->config('cookies.lifetime')) . PHP_EOL;
                $report .= 'Secure Cookie TTL:' . "\t\t\t\t\t" . ttcms_seconds_to_time($this->app->hook->{'get_option'}('cookieexpire')) . PHP_EOL;
